...
config = {
    ...
    'lr' : 1e-5,
    'save_model_every' : 0,
    'exp_name' : '3D ViT Finetune',
    'epochs' : 50,
    'model_name': 'Hybrid-Finetune',
    'chkp-file': "/home/admin1/Arindam/Alzheimer/ViT/experiments/3D ViT Final/model_best.pt"
}
...
wandb.init(
    ...
    name=f"{config['exp_name']}{datetime.datetime.now()}", 
...
def save_experiment(experiment_name, config, model, train_losses, val_losses, train_acces, val_acces, train_logs, base_dir="experiments"):
    ...
    # Save the model
    save_checkpoint(experiment_name, model, "final_finetuned", base_dir=base_dir)
...
def load_experiment(experiment_name, checkpoint_name="model_final_finetuned.pt", base_dir="experiments"):
    outdir = os.path.join(base_dir, experiment_name)
    # Load the config
    configfile = os.path.join(outdir, 'config.json')
    ...
    def train(self, trainloader, valloader, epochs, scheduler, save_model_every_n_epochs=0):
        ...
        for i in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                print(text)
                save_checkpoint(self.exp_name, self.model, 'best_finetuned')
                train_logs.append(text)
            else:
                patience_counter += 1
...
model = ViTForClassfication(config)
print(model)
model.load_state_dict(torch.load("/home/admin1/Arindam/Alzheimer/ViT/experiments/3D ViT Final/model_best_Hybrid.pt"))
...
scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[10, 20, 30], gamma=0.7)
...
config, model, train_losses, val_losses, train_acces,val_acces = load_experiment(f"../experiments/{config['model_name']}/")
...
plt.savefig(f"experiments/{config['model_name']}/metrics_fine_tune.png", dpi=600, bbox_inches='tight')
plt.show()
